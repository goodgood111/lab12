...
public class FamilyTree {
		...
		TreeNode getNodeWithName(String targetName) {
			...
			if (this.name.equals(targetName))
				...
				for (TreeNode child : children) {
					TreeNode ret = child.getNodeWithName(targetName);
					if (ret != null)
						return ret;
	...
	private void addLine(String line) throws TreeException
	{
		...
		if (colonIndex < 0) {
			throw new TreeException("Colon is not found"); // a TreeException with a useful message
		}
		...
		else
		{
			...
			if(parentNode == null) {
				throw new TreeException("parent node can't be found");
			}
		}
